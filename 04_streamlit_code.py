...
Below are plots of food insecurity rate by county in the USA. These scatter plots indicate the general food insecurity rates (in blue) and child food insecurity rates (in orange). The plots show a trend of child based food insecurity increasing at a faster rate than that of general food insecurity.  The data shows children are most at higher risk than adults for the majority of factors included in our data, most notably percent uninsured, percent minority, percent in poverty, and percent rural (shown below).''')
    ...
    def main():
        ...
        components.html(html_temp, height = 800, width = 900)
    ...
    st.write('''
The map below displays average food insecurity rates by state over time. Between 2010 and 2019 we see a general improvemnet in food insecuirty rates (going down), but many southeastern states having consistently higher rates of food insecurity than other parts of the country.
    ''')
...
A variety of factors contribute to food insecurity in a given community. Factors such as income, employment, access to healthcare, and education are just a few factors that affect one's access to food. The below visualizations depict food insecurity vs. formal education for each state in the overlying map with median household income depicted in the bar graph underneath.
    ...
    def main():
        ...
        components.html(html_temp, height = 700, width = 900)
    if __name__ == "__main__":
        main()

elif page == 'Food Insecurity Time Series Modeling':
    st.title('Food Insecurity Time Series Modeling')
    st.subheader('Modeling Food Insecurity in the United States Over Time (2010-2026)')

    st.write('''
The Facebook Prophet univariate time series model was used to forecast food insecurity by state five years into the future.
    ''')
    ...
    if __name__ == "__main__":
        ...
        st.subheader('Predicting Food Insecurity Into the Future')
        st.write('''
Below is a line graph of food insecurity. The upper line graph reflects the actual training data containing food insecurity values by state as provided by Feeding America. The lower line graph reflects the Facebook Prophet model used to forecast food insecurity five years into the future.
    ...
    if __name__ == "__main__":
        ...
        st.subheader('Multivariate Timeseries Modelling for Food Insecurity')
        st.write('''
